...
function rowScore(row: Die[]) {
  ...
  return row.reduce((sum, die) => sum + die.value * (counts[die.value] || 1), 0);
}
...
export default function Home() {
  ...
  return (
    <main className="game-shell">
      ...
      {rulesOpen && <div className="modal-backdrop" onClick={() => setRulesOpen(false)}><section className="rules-card" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setRulesOpen(false)} aria-label="닫기"><X /></button><p className="eyebrow">HOW TO PLAY</p><h2>게임 규칙</h2><ol><li><b>줄을 차지하세요</b><span>각 줄에 주사위를 3개까지 놓고, 3줄 중 2줄 이상 높은 점수를 만드세요.</span></li><li><b>더블 · 트리플 보너스</b><span>같은 숫자 2개는 합계 ×2, 3개는 합계 ×3입니다.</span></li><li><b>알까기</b><span>일반 주사위를 내 줄에 놓으면 상대 같은 줄의 동일 숫자 일반 주사위를 모두 제거합니다.</span></li><li><b>실드 주사위</b><span>알까기에 제거되지 않으며, 보상 실드는 양쪽 보드 어디든 놓을 수 있습니다.</span></li><li><b>세 가지 난이도</b><span>초보는 무작위 중심, 숙련은 알까기와 점수를 비교하고, 전문가는 줄의 위협도와 콤보까지 계산합니다.</span></li><li><b>초보자 승률 안내</b><span>남은 주사위 조합을 계산해 줄별 예상 승률과 추천 줄을 표시합니다.</span></li></ol></section></div>}
    </main>
  );
}
